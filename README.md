...
* **One-Click Conversation Sharing & Native OS Integration**: Generate clean, shareable web links for any conversation turn with customizable expiration dates (1, 7, 30, 90 days, or permanent), preserving full markdown formatting, thinking processes, and attached images. Deeply integrated with native mobile share sheets (Android Capacitor and Web Share API) with protocol normalization for social messengers (WeChat, Twitter/X, Telegram) to automatically render rich cards with edge-generated preview thumbnails (OpenGraph). Recipients can read seamlessly on any device or import the conversation into their own ZenChat with one click.
...
10. **一键对话快照与系统原生分享**：支持将任意轮次的精彩问答一键发布为在线网页链接，支持自选有效期（1天、7天、30天、90天或永久有效）；完整保留排版、思考过程与配图。深度打通移动端系统原生分享底栏（Android 原生 Intent 与 Web Share API），遵循标准化 URL 协议，在微信、Telegram、Twitter/X 中分享自动展开带有标题、摘要与动态预览缩略图的精美大卡片；好友无需注册即可在任意设备浏览，或一键导入到自己的 ZenChat 中继续追问。
