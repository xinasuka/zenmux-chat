...
### 3. Mobile PWA & Standalone Android APK
...
* **Compile Standalone Android APK**:
  ```bash
  npm run build:apk
  ```
  The compiled APK is emitted at `android/app/build/outputs/apk/debug/app-debug.apk`.
...
### 移动端 PWA 与 Android APK 编译

* **iOS (Safari 浏览器)**：访问站点，点击分享按钮并选择「添加到主屏幕」即可作为全屏独立应用使用，无需苹果开发者证书，永不过期。
* **Android (Chrome 浏览器)**：访问站点，点击菜单选择「添加到主屏幕」或「安装应用」。
* **本地编译独立 Android APK**：
  ```bash
  npm run build:apk
  ```
  编译完成的 APK 安装包生成于 `android/app/build/outputs/apk/debug/app-debug.apk`。
